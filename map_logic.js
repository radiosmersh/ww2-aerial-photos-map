...
const GEOJSON_FILE_PATHS = [
    'scans_nara.geojson',
    'scans_ign.geojson',
    'scans_barch.geojson' // Added new GeoJSON source
];
...
function createBundesarchivPopupContent(properties) {
    ...
    // You can add an image preview if Bundesarchiv provides a direct image link or IIIF
    // For example, if 'direct_link' pointed to a .jpg or .png:
    // if (properties.direct_link && /\.(jpeg|jpg|gif|png)$/i.test(properties.direct_link)) {
    //    content += `<img src="${properties.direct_link}" alt="Preview" class="popup-image">`;
    // }
...
function createPopupContent(properties) {
    ...
    } else if (properties["provenance"] == "ign") { // IGN (often has mission_id, may or may not have image_id for GeoTIFF)
    ...
    return L.geoJSON(geoJsonFeatureCollection, {
        onEachFeature: function (feature, layer) {
            if (feature.properties) {
                ...
                // No specific 'popupopen' handler needed for Bundesarchiv yet, unless it involves async preview loading.
            }
        }
    });
}
